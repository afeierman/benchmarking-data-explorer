...
shinyServer(
  ...
  ed <- reactive({
    clean_bm %>% 
      select(City = city, Year, "Zip Code" = Zip.Code, "Site EUI" = SiteEUI, 
             "Source EUI" = SourceEUI, "Weather Normalized Site EUI" = NormSiteEUI,
  ...
  output$graph1 <- renderPlot({
    ...
    data_explorer <- ggplot(data = plot_check, aes(x = plot_check[input$xvar], y = plot_check[input$yvar], alpha = .05), size = .1) +
      geom_point(aes()) + 
      graph1xlims() +
      graph1ylims() +
      ...
      scale_color_discrete(name = "City", 
                           breaks = c(input$show_cities), 
                           labels = c(input$show_cities)) +
      guides(alpha = FALSE) + 
      theme(axis.text = element_text(size = 13),
            axis.title = element_text(size = 14))
    
    if (input$yearcolor == "Year") {
      data_explorer <- data_explorer + geom_point(aes(color = factor(Year), 
                                alpha = (1/length(input$show_cities)))) + 
        scale_color_discrete(name = "Year", breaks = c(input$show_years), 
                             labels = c(input$show_years))
    }
    if (input$yearcolor == "City") {
      data_explorer <- data_explorer + geom_point(aes(color = factor(city), 
                                alpha = (1/length(input$show_cities))))
    }
    if (input$trendline == TRUE) {
      data_explorer <- data_explorer + geom_smooth(aes(group = city, 
                                 color = city, 
                                 fill = factor(city))) + 
        scale_fill_manual(values = c("orange","purple", "red"), name = "Trendline(s)")
    }
    data_explorer
  })
  ...
  colorpal <- reactive({
    colorQuantile(colorRamp(c("#0000FF", "#FF0000"), 
                            interpolate = "spline"), full_zips$MedSourceEUI, n = 10)
  })

  output$map <- renderLeaflet({
    leaflet(filteredData()) %>% addTiles() %>%
      setView(-73.90, 40.7128, zoom = 12) %>% 
      addCircles(~longitude, ~latitude, weight = 5, 
                 color = ~qpal(MedNormSourceEUI), 
                 fillColor = ~qpal(MedNormSourceEUI), 
                 fillOpacity = .9, 
                 popup = ~paste("Source EUI:", MedNormSourceEUI, " Zip Code:", zip), 
                 radius = ~full_zips$Count) %>%
...
?addLegend
  observe({
    ...
    leafletProxy("map", data = filteredData()) %>%
      ...
      addCircles(~longitude, ~latitude, color = ~qpal(MedNormSourceEUI), 
                 weight = 5, fillColor = ~qpal(MedNormSourceEUI), 
                 fillOpacity = .9, 
                 popup = ~paste("Zip Code:", zip, "\n"," Source EUI:", MedNormSourceEUI),
                 radius = ~full_zips$Count * 5)
  })
  
  #customize data table output
  cityinfo <- reactive({
    clean_bm %>% 
      select(City = city, Year, "Zip Code" = Zip.Code, 
             "Site EUI" = SiteEUI, "Source EUI" = SourceEUI, 
